...
import Category_Edit from "./Inventory_Edit";
...
function InventoryList() {
  ...
  const columns = [
    {
      ...
      sort: true,
      formatter: (cell, row) => {
        return <div>{cell ? moment(cell).format('MM-DD-YYYY MM:SS') : "-"}</div>;
      },
    },
    {
      dataField: "weight",
    ...
    {
      dataField: "buyerPartyId",
      text: lan.buyerPartyId,
      sort: true,
      formatter: (cell, row) => {
        return <div>{cell.length>0 ? cell[0]?.name : "-"}</div>;
      },
    }, {
      dataField: "brokerPartyId",
      text: lan.brokerPartyId,
      sort: true,
      formatter: (cell, row) => {
        return <div>{cell.length>0 ? cell[0]?.name : "-"}</div>;
      },
  ...
  const fetchData = async (page, limit, search) => {
    ...
    await ApiPost("/inventory/get", body)
      .then((res) => {
        console.log("resData", res?.data);
        setData(res?.data?.data?.inventory_count);
  ...
  return (
    <>
      <div className="d-flex justify-content-between mb-4">
        <div className="title">
          <div className="fs-20px fw-bolder">{lan.sidelist_inventory}</div>
          <div>
            <span
              role="button"
      ...
      <div class="d-flex flex-column flex-column-fluid h-100" id="kt_content">
        ...
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
              <h3 class="card-label">
              {lan.sidelist_inventory} {lan.list}
              </h3>
            </div>
            <div class="card-toolbar">
              <a
                ...
                onClick={() => openModal()}
              >
                {lan.add} {lan.sidelist_inventory}
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title id="example-modal-sizes-title-lg">
                {lan.delete} {lan.sidelist_inventory}
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <span>
                {lan.are_you_sure} {lan.sidelist_inventory}?
              </span>
            </Modal.Body>
            <Modal.Footer>
              <div>
