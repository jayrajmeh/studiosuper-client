...
import Category_Edit from "./Kapan_Edit";
...
function KapanList() {
  ...
  const columns = [
    ...
    
    {
      ...
      sort: true,
      formatter: (cell, row) => {
        return <div>{cell==0?lan.galaxy:lan.manual}</div>;
  ...
  const fetchData = async (page, limit, search) => {
    ...
    await ApiPost("/kapan/get", body)
      .then((res) => {
        console.log("resData", res?.data);
        setData(res?.data?.data?.kapan_count);
  ...
  return (
    <>
      <div className="d-flex justify-content-between mb-4">
        <div className="title">
          <div className="fs-20px fw-bolder">{lan.sidelist_kapan}</div>
          <div>
            <span
              role="button"
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title id="example-modal-sizes-title-lg">
                {lan.delete} {lan.sidelist_kapan}
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <span>
                {lan.are_you_sure} {lan.sidelist_kapan}?
              </span>
            </Modal.Body>
            <Modal.Footer>
              <div>
