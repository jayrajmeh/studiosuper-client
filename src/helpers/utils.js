...
import { BaseURL, Bucket } from "./API/ApiData";
...
export class Uploader {
  constructor(options) {
    ...
    this.chunkSize = options.chunkSize || 1024 * 1024 * 1024;
  ...
  async initialize() {
    try {
      ...
      const initializeReponse = await api.request({
        url: "/upload/initializeMultipartUpload",
        method: "POST",
      ...
      const urlsResponse = await api.request({
        url: "/upload/getMultipartPreSignedUrls",
        method: "POST",
  ...
  async sendCompleteRequest() {
    if (this.fileId && this.fileKey) {
      ...
      await api.request({
        url: "/upload/finalizeMultipartUpload",
        method: "POST",
      ...
      this.onSuccessFn();
      // console.log("urldata", videoFinalizationMultiPartInput?.fileKey);
    }
  }
